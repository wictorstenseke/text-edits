...
1. **Always update both languages**: When adding or changing UI text, update both `en` and `sv` translation files. See [`AGENTS.md`](../AGENTS.md) for project rules.
2. **Keep keys descriptive**: Use semantic keys like `header.resetDocument` instead of generic keys like `button1`
3. **Group by feature**: Organize translations by component or feature for easier maintenance
4. **Be consistent**: Use the same translation key structure across all language files
5. **Avoid hardcoded strings**: Always use the translation function `t()` for user-facing text
6. **Test all languages**: Verify that all strings are properly translated and the UI renders correctly in all supported languages
